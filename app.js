...
app.configure(function() {
	app.set('port', process.env.PORT || 3000); // process.env.OPENSHIFT_NODEJS_PORT
  	app.set('ipaddr', process.env.VCAP_APP_HOST || "127.0.0.1"); // process.env.OPENSHIFT_NODEJS_IP
  	console.log(app.get('ipaddr'));
	...
	*/
	if (people[s.id].inroom) { //user is in a room
		...
		if (s.id === room.owner) { //user in room and owns room
			if (action === "disconnect" && room.people.length === 0) {
